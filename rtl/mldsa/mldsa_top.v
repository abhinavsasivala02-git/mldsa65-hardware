...
module mldsa_top (
    ...
    // Write: wr_en + wr_addr + wr_data sampled on posedge (1-cycle).
    // Read : rd_en + rd_addr sampled on posedge, rd_data valid next cycle.
    input  wire          wr_en,
    ...
    // Several combinational readers each, so these cannot be BRAM: a block
    // RAM read is registered and would add a second cycle to accesses the
    // FSMs budget one cycle for.
    // ponytail: LUTRAM keeps the cycle counts and the KATs intact at the
    // cost of ~11k LUTs; BRAM means registering reads and adding FSM waits.
    (* ram_style = "distributed" *) reg [7:0]  pk_ram [0:1951];  // 1952 bytes: rho(32) + t1(1920)
    (* ram_style = "distributed" *) reg [7:0]  sk_ram [0:4031];  // 4032 bytes: rho(32)+K(32)+tr(64)+s1(1280)+s2(1536)+t0(704)
    (* ram_style = "distributed" *) reg [7:0]  sig_ram [0:3311]; // 3309 bytes + pad to 32-bit boundary (0x3000-0x34EF)
    ...
    // Controller writes to pk/sk/sig live in the host write block below:
    // one always block per memory, or synthesis rejects the multi-driver array.
    ...
    // Verify signature byte read port (sig_ram)
    wire [11:0] vf_sig_addr_w;
    wire [7:0]  vf_sig_rdata_w = sig_ram[vf_sig_addr_w];

    // Verify pk byte read port (rho + t1 from pk_ram)
    ...
    // intt_mode is a LEVEL (not a pulse): only the active controller's value
    ...
    // Controllers have write priority over the host; the host is not expected
    always @(posedge clk) begin
        if (kg_pk_we_w)  pk_ram [kg_pk_addr_w]  <= kg_pk_wdata_w;
        else if (wr_en && wr_pk)  pk_ram [wr_addr[10:0]] <= wr_data[7:0];

        if (kg_sk_we_w)  sk_ram [kg_sk_addr_w]  <= kg_sk_wdata_w;
        else if (wr_en && wr_sk)  sk_ram [wr_addr[11:0]] <= wr_data[7:0];

        if (sg_sig_we_w) sig_ram[sg_sig_addr_w] <= sg_sig_wdata_w;
        else if (wr_en && wr_sig) sig_ram[wr_addr[11:0]] <= wr_data[7:0];
