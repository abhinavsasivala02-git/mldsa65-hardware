...
module sign_ctrl (
    ...
    // SK RAM byte read port (combinational in top)
    ...
    localparam [22:0] GAMMA1    = 23'd524288;   // 2^19
    localparam [22:0] GAMMA2    = 23'd261888;
    localparam [22:0] BETA      = 23'd196;
    localparam [22:0] Z_BOUND   = GAMMA1 - BETA;
    localparam [22:0] Q_MINUS_Z = `MLDSA_Q - Z_BOUND;
    localparam [22:0] R0_BOUND  = GAMMA2 - BETA;
    localparam [22:0] Q_MINUS_R0= `MLDSA_Q - R0_BOUND;
    localparam [22:0] CT0_BOUND = GAMMA2;
    localparam [22:0] Q_MINUS_CT0 = `MLDSA_Q - CT0_BOUND;
    localparam [22:0] Q_HALF    = 23'd4190209;
    localparam [22:0] Q_2G      = `MLDSA_Q - 2*GAMMA2;
    localparam [22:0] Q_G       = `MLDSA_Q - GAMMA2;

    localparam [4:0] L = `MLDSA_L;
    localparam [4:0] K = `MLDSA_K;
    ...
    // eta nibble: nib = ETA - c, c stored mod Q
    ...
    // t0 coeff from 13-byte group: v = (1<<12) - a (offset), coeff = (1<<12) - v mod Q
    ...
    // polyz pack value: t20 = GAMMA1 - z (z stored mod Q), 0..2^20-1
    ...
    // polyz unpack coeff: GAMMA1 - c20 (c20 0..2^20-1), mod Q
    ...
    // SampleInBall challenge byte address: pos<136 in slot 40, else slot 41
    ...
    reg [2:0]  mat_i, mat_j;
    reg [3:0]  poly_idx;
    reg [8:0]  coeff_idx;      // pipelined coeff counter (0..263)
    reg [8:0]  coeff_cnt;
    reg [15:0] kappa;
    ...
    always @(posedge clk) begin
        if (!rst_n) begin
            ...
            // STEP 2a: unpack s1 (L x 128B, nibble eta) -> slots 10..14
            SG_UP_S1: begin
                case (sub)
                5'd0: begin
            ...
            // STEP 2b: unpack s2 (K x 128B, nibble eta) -> slots 15..20
            SG_UP_S2: begin
                case (sub)
                5'd0: begin
            ...
            // STEP 2c: unpack t0 (K x 416B, 8x13-bit) -> slots 21..26
            SG_UP_T0: begin
                case (sub)
                // Read 13 bytes (sk_ram combinational: present at sub, capture next)
                5'd0: begin
                    sk_rd_addr <= 12'd1536 + poly_idx * 12'd416 + up_byte*12'd13 + sub;  // byte = grp*13 + 0
                    sub <= 5'd1;
                end
            ...
            // STEP 3: y = ExpandMask(rho'', kappa)  (L polys, 640B each)
            SG_EM_INIT: begin
                shake_init <= 1'b1;
            ...
            // Consume one byte from current lane, accumulate into group buffer
            SG_EM_BYTE: begin
            ...
            // FEED from reading stale (pre-permute) state.
            ...
            // Pointwise multiply + accumulate: acc[i][c] += A[j][c] * y_hat[j][c]
            // Spad reads: addr present at posedge c -> data valid at posedge c+2.
            // Montgomery mult: feed at c (data c-2) -> vld_out at c+5 = product[c-7].
            //   capture at c: madd_a/b = ACC[c-7]/product[c-7], pmul_wr_idx = c-7
            //   write at c+1 : ACC[c-7] = madd_sum
            SG_MM_PMUL_PIPE: begin
                ...
                if (pmul_vld_out) begin
                    ...
                    pmul_wr_idx    <= coeff_idx[7:0] - 8'd7;
                    pmul_acc_phase <= 1'b1;
                end else begin
                    pmul_acc_phase <= 1'b0;
                end
            ...
            // STEP 5: (w1, w0) = Decompose(w) per row
            SG_MM_DECOMP: begin
                if (coeff_idx <= 9'd255) begin
                ...
                if (coeff_idx >= 9'd3 && coeff_idx <= 9'd258) begin
                    sp_wr_en <= 1'b1;
                    if (sub == 5'd0) begin
                        // pass 0: write w1 to SP_W1, leave w intact in SP_W0
                        sp_wr_addr <= base_addr(SP_W1 + mat_i) + (coeff_idx - 9'd3);
                        sp_wr_data <= dc_r1;
                    end else begin
                        // pass 1: overwrite w in SP_W0 with w0 (w already consumed)
            ...
            // Zero challenge slot (c[0..206] must be 0)
            SG_SIB_ZERO: begin
                sp_wr_en   <= 1'b1;
            ...
            // STEP 8: z = y + INTT(c_hat * s1_hat), norm check
            ...
            // Store INTT result + y -> z, check norm
            SG_Z_STORE: begin
                if (coeff_idx <= 9'd255) begin
                    ntt_ext_addr <= coeff_idx[7:0];
            ...
            // STEP 9: r0 = w0 - INTT(c_hat * s2_hat), norm check
            SG_PM_C2: begin
                if (coeff_idx <= 9'd255) begin
                    sp_rd_addr  <= base_addr(SP_C) + coeff_idx;
            ...
            SG_R0_STORE: begin
                ...
                if (coeff_idx >= 9'd2 && coeff_idx <= 9'd257) begin
                    madd_a <= sp_rd2_data;
                    madd_b <= ntt_ext_dout;      // INTT result (cs2)
                end
                if (coeff_idx >= 9'd3 && coeff_idx <= 9'd258) begin
                    sp_wr_en   <= 1'b1;
                    sp_wr_addr <= base_addr(SP_W0 + poly_idx) + (coeff_idx - 9'd3);
                    sp_wr_data <= madd_diff;
                    if (!((madd_diff < R0_BOUND) || (madd_diff > Q_MINUS_R0)))
                        r0_fail <= 1'b1;
                end else begin
                    sp_wr_en <= 1'b0;
                end
            ...
            // STEP 10: ct0 = INTT(c_hat * t0_hat), norm check, w0 += ct0
            SG_PM_C3: begin
                if (coeff_idx <= 9'd255) begin
                    sp_rd_addr  <= base_addr(SP_C) + coeff_idx;
            ...
            SG_CT0_STORE: begin
                ...
                if (coeff_idx >= 9'd3 && coeff_idx <= 9'd258) begin
                    ...
                    sp_wr_addr <= base_addr(SP_W0 + poly_idx) + (coeff_idx - 9'd3);
                    sp_wr_data <= madd_sum;
                    if (!((madd_a < CT0_BOUND) || (madd_a > Q_MINUS_CT0)))
                        ct0_fail <= 1'b1;
                end else begin
                    sp_wr_en <= 1'b0;
                end
            ...
            SG_HINTS: begin
                ...
                if (coeff_idx == 9'd258) begin
                    coeff_idx <= 9'd0;
                    // coeff 255 (the final one) is evaluated this cycle; its
                    // hint increment must be included in the stored count.
